...
"Chat gpt answer: To create a boxplot using ggplot2 in R, especially with the gapminder data for the year 2007, and to not display the outliers, you can modify the geom_boxplot() function by adding an argument that controls the outlier shape. Setting outlier.shape = NA will prevent the outliers from being displayed in your plot.

The three variables are continent, lifeExp and I suppose fill is also defined as continent, therefore fill"
```
...
"geom_bar() figures out the counts on its own and shows us a chart based on that, while geom_col() needs us to do the counting first and then it shows the chart based on the numbers we give it."
```
...
"They both create bars for plots and they both show distribution of data

increasing the last argument in break() results in fewer but larger bars -> makes it easier to read but we don't have as many details"
```
